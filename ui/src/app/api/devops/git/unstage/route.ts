...
async function executeGitUnstage(
  node: string,
  files: string[]
  ...
  return new Promise((resolve, reject) => {
    ...
    if (node === "lovelace") {
      const command = `git -C "${repoPath}" restore --staged -- ${quotedFiles}`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await executeGitUnstage(node, files as string[]);
    ...
    return NextResponse.json(
      { error: "Git unstage failed", details: String(error) },
      { status: 500 }
    );
  }
}
