 ...
 *
 * Design language:
 *  - Mirrors the Cowork task-list UX: objective at top, ordered plan steps
 *    with pending/spinning/done indicators, evidence section, progress bar.
    ...
    <>
      ...
      <button
        ...
        className={cn(
          ...
          "bg-[var(--surface,#1a1b2e)] shadow-lg transition-all",
      ...
      >
        ...
        <span
          className="text-[9px] font-semibold uppercase tracking-widest [writing-mode:vertical-rl] rotate-180"
        >
          Goals
        </span>
      ...
      <div
        className={cn(
          ...
          "flex flex-col border-l border-[var(--border,rgba(255,255,255,0.1))]",
          "bg-[var(--surface,#1a1b2e)] shadow-xl",
          "transition-transform duration-300",
          panelOpen ? "translate-x-0" : "translate-x-full",
        )}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border,rgba(255,255,255,0.08))]">
          ...
          <div className="flex items-center gap-1.5">
            {/* Status badge */}
            <span className={cn("text-[10px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full border", statusColor)}>
              {activeGoal.status}
            </span>
            <button
          ...
          {evidence.length > 0 && (
            <div>
              <button
                ...
                className="flex items-center gap-1.5 text-[10px] text-[var(--chat-subtle)] uppercase tracking-[0.12em] font-semibold hover:text-[var(--chat-fg)] transition-colors w-full"
              >
                <svg
              ...
              {showEvidence && (
                <div className="mt-2 flex flex-col gap-1.5">
                  {evidence.map((ev) => (
                    <div
                      key={ev.id}
                      className="flex items-start gap-2 p-2 rounded-lg bg-[var(--surface-elevated,rgba(255,255,255,0.04))] border border-[var(--border,rgba(255,255,255,0.06))]"
                    >
                      <span className="text-[9px] font-bold uppercase tracking-wide text-[var(--chat-subtle)] bg-[var(--border,rgba(255,255,255,0.08))] rounded px-1.5 py-0.5 flex-shrink-0 mt-0.5">
          ...
          <div className="px-4 py-3 border-t border-[var(--border,rgba(255,255,255,0.08))] flex gap-2">
            {!isPaused && (
              <button
                ...
                className="flex-1 py-1.5 text-xs rounded-lg border border-[var(--border)] text-[var(--chat-subtle)] hover:text-[var(--chat-fg)] hover:border-[var(--chat-subtle)] transition-colors"
              >
                Pause
              </button>
            )}
            {isPaused && (
              <button
                onClick={async () => {
                  // Re-activate: just re-ensure
                }}
                className="flex-1 py-1.5 text-xs rounded-lg border border-amber-500/30 text-amber-400 hover:bg-amber-500/10 transition-colors"
              >
                Paused
              </button>
            )}
            <button
              ...
              className="flex-1 py-1.5 text-xs rounded-lg bg-green-500/15 border border-green-500/25 text-green-400 hover:bg-green-500/25 transition-colors font-medium"
            >
              Complete ✓
            </button>
          </div>
