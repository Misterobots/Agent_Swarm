 ...
 *
 * Each SVG uses `currentColor` so it inherits the text-{color} class
 * already applied to its parent avatar container.
 *
 * Role → Pioneer mapping (from lamport.py WORKER_PIONEERS):
 *   researcher → Claude Shannon    (neat side-part, 1940s clean-cut)
 *   architect  → Charles Babbage   (Victorian mutton-chop sideburns, high collar)
 *   coder      → Donald Knuth      (iconic round glasses, beard, balding crown)
 *   devops     → Vint Cerf         (full bushy beard & moustache, suit + tie)
 *   analyst    → Edgar Codd        (1970s side-part, tidy academic)
 *   verifier   → Tony Hoare        (rectangular glasses, distinguished silver)
 */
