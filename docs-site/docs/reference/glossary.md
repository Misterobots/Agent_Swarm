...
| **Control Node** | The server ({{ hopper_ip }}) running SPIRE, PostgreSQL, and Langfuse |
...
| **Gateway Node** | The Turing server ({{ turing_ip }}) running Traefik (`babbage`), Prometheus, Grafana (`hollerith`), and Loki |
...
| **jacquard** | Metrics collection and alerting system |
