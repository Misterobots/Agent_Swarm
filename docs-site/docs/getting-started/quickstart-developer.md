...
ollama pull qwen3:8b                    # Router (intent classifier)
...
|   +-- semantic_router.py   # Intent classifier (15 categories, qwen3:8b + 5-rule fast-path)
|   +-- handlers/            # One module per intent group (creative, image, devops, etc.)
|   +-- routing/             # Multi-turn pending-context gates (gates.py)
|   +-- mars_loop.py         # MarsRL: Solver -> Verifier -> Corrector
|   +-- lamport.py           # Coordinator entry point (re-exports coordination/)
|   +-- coordination/        # Decomposer, executor, synthesizer, palace, pioneers, session
...
| `agents/church.py` | Thin router wrapper — intent dispatch, session init, ctx dict |
| `agents/semantic_router.py` | Intent classifier — 15 categories, qwen3:8b LLM + 5-rule keyword fast-path |
| `agents/handlers/` | Intent handler modules — one file per intent group |
| `agents/mars_loop.py` | MarsRL quality loop — Solver -> Verifier -> Corrector |
| `agents/lamport.py` | Coordinator entry point — re-exports `agents/coordination/` |
