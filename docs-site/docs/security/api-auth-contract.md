---
title: API Authentication and Claims Contract
---

# API Authentication and Claims Contract

Defines endpoint classes, token profile requirements, and claims validation rules for the Home AI Lab APIs.
...
- **Required claims**: `iss`, `sub` (SPIFFE ID), `aud`, `exp`, `iat`, `nbf`
...
- **Required claims**: `iss`, `sub` (user_id), `aud`, `exp`, `iat`, `nbf`, `jti`
- **Optional claims**: `roles`, `scope`, `session_id`
...
- [API Contract Validation Examples](api-auth-examples.md) — executable request/response examples demonstrating this contract
- [ADR-001: JWT-ACE Profiles over RBAC](../architecture/decisions/adr-001-jwt-profiles.md) — the architectural decision behind the User Access Token (JWT-ACE) profile and its token trust rules
- `agents/main.py`
