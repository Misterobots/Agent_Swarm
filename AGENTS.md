...
- [ ] **Pairing end-to-end test needs a human** — the relay protocol now has local two-socket coverage (`tests/test_pairing_relay.py`) and the WS routing fix/Traefik `pairing-ws` router were verified by config/log inspection. Authentik's forward-auth intercepts every path identically, so deployed behavior still needs two authenticated browser sessions (host + guest) to confirm the full relay.
- [x] **"Agent View" live session registry** — reviewed and verified. `agents/coordination/session.py` publishes weakref-backed live sessions and workers, `/api/v1/swarm/sessions` exposes the snapshot, and `tests/test_agent_view.py` covers serialization plus collected-session cleanup (2026-08-27).
- [ ] **`-Justin-PC` fork file sprawl + worktree sprawl** — triaged 2026-08-27: 28 tracked `*-Justin-PC*` files were compared with their canonical counterparts; 7 are byte-identical after CRLF normalization and 21 diverge. Divergences include BMO audio input, older router/runtime code, historical repair scripts, and the intentional Turing compose variant. Git currently registers 8 worktrees. No files or worktrees were deleted or merged without dedicated owner review.
- [x] **Docs-site and ntfy routing/config review** — independently verified 2026-08-27. The running docs container's external HTTP/HTTPS routes carry `authentik@file` and an unauthenticated host-routed request returns `302` to Authentik. Canonical ntfy compose requires `NTFY_ADMIN_USER`, `NTFY_ADMIN_PASSWORD_HASH`, and `NTFY_ALERT_TOKEN`; the running ntfy container is healthy and has a non-default bcrypt admin hash.
- [x] **Android build pipeline** — profile-gated builder, bounded disposable build, owner-scoped route, and signed APK handoff implemented and verified with a live minimal Android fixture; `assembleRelease` succeeded and `apksigner` verified v1/v2 signatures (2026-08-26).
- [x] **Dev workspace session continuity review** — fixed the first-session response contract: the UI now reads the server's `{session: {...}}` envelope and persists the returned session ID, allowing subsequent saves and cross-device hydration (2026-08-27).
- [x] **Dev UI/UX design review** — authenticated `/dev` review completed: Editor, Terminal, Goals, and Notes controls respond, but docked panels can collapse into unusable narrow right-edge slivers at the tested viewport. The selected project was empty, so no project-specific content was created during review (2026-08-26).
- [ ] **Visual UI testing of dev workspace** — authenticated `/dev` review verified Agent Team, Goals/TodoCard empty-step state, Notes, and Agent Trace controls. Diff chips and project-specific pioneer/trace rendering still require an already-owned populated project or an approved test fixture; no project data was created or modified (2026-08-27).
