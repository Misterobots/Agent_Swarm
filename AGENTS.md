...
- [ ] **Android build pipeline** — scoped but not implemented: the current `dev-sandbox` image has no Java, Gradle, Android SDK, or APK artifact handoff. Implement as a dedicated Android build image/tool (rather than enlarging the general sandbox) with bounded build resources and an authenticated artifact path; triggered by the Hitchhiker's Guide project.
