...
- [ ] **Android build pipeline** — implementation added in an isolated, profile-gated Android builder with bounded resources, owner-scoped build validation, and signed APK handoff (`agents/dev_android/`, `execution_plane/Dockerfile.android-builder`). Controlled image build plus one authenticated Android fixture remain pending: Lovelace Docker Desktop's `desktop` credential helper fails from the SSH logon session (`A specified logon session does not exist`); the live `main` checkout was left untouched (2026-08-26).
