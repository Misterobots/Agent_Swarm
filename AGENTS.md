...
- [x] **`-Justin-PC` fork file sprawl + worktree sprawl** — completed 2026-08-28 and documented in `docs/procedures/justin-pc-variant-disposition.md`: all unreferenced non-deployment variants were removed, leaving only the intentional live Turing gateway compose variant. Eight redundant clean worktree directories were removed while retaining their branch refs; the dirty primary `main` worktree was preserved.
