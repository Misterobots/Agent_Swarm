...
- [ ] **Turing small-model warm latency is higher than expected (generic fast path, not Friday)** — `llama3.2:3b` measured ~10s warm-inference for a few tokens on the 3070Ti. GPU is confirmed engaged (100% VRAM via `/api/ps`), so this isn't a CPU fallback — likely a tuning/contention question, worth investigating before relying on the fast path for latency-sensitive calls.
