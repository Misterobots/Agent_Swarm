...
| **Lovelace** | `192.168.2.101` | **Local workstation** — where you and Codex run. Execution plane, GPU node, ComfyUI, Ollama. |
...
> **This is the machine you and Codex are running on.** It is not a remote server — it is the local workstation. Run Docker and shell commands here directly; no SSH needed.
...
```
Agent_Swarm/
├── AGENTS.md                  ← you are here (session primer)
├── agents/
...
- [x] **BMO voice model/personality retirement** — the obsolete `BMO_LLM_MODEL=qwen3:14b` configuration and standalone BMO character layer were removed; Friday remains on `qwen3:8b`, while the physical voice/TTS infrastructure was preserved (2026-08-25).
- [x] **Runtime/backend/gateway integration preflight** — runtime lifecycle, permission, backend handoff, and canonical gateway-env changes merge cleanly on the combined review branch; focused tests, compilation, and diff validation passed (2026-08-25).
- [x] **Controlled runtime deployment** — deployed the combined runtime revision through a disposable staging worktree and isolated Compose override; `agent_runtime` is stable with Redis connected, while `memex_ui`, Traefik, and Cloudflared remained untouched and healthy (2026-08-26).
- [x] **Turing small-model latency investigation** — live Ollama verification showed a ~22.8s cold model load followed by a warm `llama3.2:3b` request completing in ~42ms (HTTP 200); all 29 model layers were GPU-offloaded on the RTX 3070 Ti, so no routing or tuning change was required (2026-08-26).
- [x] **Pairing end-to-end verification** — the live root Compose file was missing the reviewed `pairing-ws` Traefik labels; after backing it up and recreating only `memex_ui`, two authenticated sessions connected successfully. The join returned HTTP 200 and the runtime accepted the `WebSocket /api/v1/pairing/ws/...` upgrade (2026-08-26).
- [x] **"Agent View" live session registry** — reviewed the `_ACTIVE_SESSIONS` weakref registry, worker serialization, `/api/v1/swarm/sessions` endpoint, and UI consumer; an isolated contract test confirmed registration, worker state/counts, and automatic cleanup after session collection (2026-08-26).
- [x] **`-Justin-PC` fork file sprawl + worktree sprawl triage** — review branch inventory found 28 tracked variants (8 byte-identical, 20 differing, no orphaned variants); Turing has 67 on its long-lived dirty checkout plus active reconciliation worktrees. The live gateway variant was identified; no files or worktrees were deleted because the remaining copies require owner-by-owner approval (2026-08-26).
- [x] **Docs-site and ntfy configuration review** — confirmed and fixed the docs authentication gap in the live root Compose (LAN, HTTPS, and Cloudflare HTTP routers now use Authentik; the HTTP route was missing, allowing tunnel traffic to fall through). Updated the tracked Justin-PC variant to require `NTFY_ADMIN_PASSWORD_HASH` and `NTFY_ALERT_TOKEN` instead of fallback credentials; docs-site rebuilt and is running (2026-08-26).
- [x] **Android build pipeline** — isolated profile-gated Android builder completed and validated: Lovelace built the image from the feature branch; Java 17, Gradle 8.10.2, SDK tooling, and a minimal Android fixture produced `app-release-unsigned.apk` successfully in 36 seconds under the 8 GB/4 CPU/512-process limits. The disposable fixture container was removed afterward; the live `main` checkout and execution services were untouched (2026-08-26).
- [x] **Dev workspace session continuity review** — found and fixed the first-session contract mismatch where the UI read `created.id` although the backend returns `{session: {...}}`; the persisted session ID is now stored, allowing subsequent state saves and cross-device hydration (2026-08-26).
- [x] **Dev UI/UX design review** — authenticated review found that simultaneously
  visible right-docked panels each requested 40% width, collapsing Editor/Goals/Notes
  into unusable slivers; committed bounded shared sizing in `728c8230` (2026-08-26).
- [ ] **Visual UI testing of dev workspace** — authenticated baseline pass completed: `/dev` opens, Editor/Terminal/Goals/Notes render, persisted goals/notes load, and no browser console warnings/errors appeared. Project-specific TodoCard, diff chips, pioneer cards, and agent-trace states remain open because no active dev project was available; none was created solely for testing (2026-08-26).
