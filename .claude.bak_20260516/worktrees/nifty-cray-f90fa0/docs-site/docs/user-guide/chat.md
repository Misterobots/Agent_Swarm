...
The Semantic Router (qwen3:8b LLM + 5-rule keyword fast-path) classifies your message and routes it to the appropriate agent. You don't need to specify the intent — it's detected automatically.

The LLM retry threshold is **0.75** — below that the router retries with a stronger prompt (max 2 retries). A confidence gate at **0.80** applies after routing: if confidence is still below the gate, the system shows a clarification card asking you to confirm intent rather than guessing. Five intents bypass this gate (VISION, ACTION_FIGURE, IOT_CONTROL, IOT_DEV, TRAIN) because they match via high-precision keyword rules.
