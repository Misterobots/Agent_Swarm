...
async function executeGitPush(node: string): Promise<{ stdout: string; stderr: string; exitCode: number }> {
  return new Promise((resolve, reject) => {
    ...
    const command = `cd ${repoPath} && git push`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await executeGitPush(node);
    ...
    return NextResponse.json(
      { error: "Git push failed", details: String(error) },
      { status: 500 }
    );
  }
}
