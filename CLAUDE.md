...
- [ ] **`-Justin-PC` fork file sprawl + worktree sprawl** — triaged 2026-08-27: 28 tracked `*-Justin-PC*` files resolve to 19 normalized-identical copies and 9 semantic variants. Variants include BMO audio input, older router/runtime code, and the intentional Turing compose variant; no files were deleted or merged without dedicated review. Worktree sprawl remains uncounted in this checkout.
