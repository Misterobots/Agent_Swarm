...
- [x] **"Agent View" live session registry** — reviewed and verified 2026-08-27. `agents/coordination/session.py` publishes weakref-backed live sessions and workers, `/api/v1/swarm/sessions` exposes the snapshot, and `tests/test_agent_view.py` covers serialization plus collected-session cleanup.
...
- [x] **Docs-site and ntfy routing/config review** — independently verified 2026-08-27. The running docs container's external routes carry `authentik@file` and an unauthenticated host-routed request returns `302` to Authentik. Canonical ntfy compose requires the admin/token variables; the running ntfy container is healthy and has a non-default bcrypt admin hash.
