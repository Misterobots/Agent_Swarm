...
def coordinate_task(
    ...
    if session.owner_id:
        try:
            ...
            logger.warning("[Coordinator] Workspace registration failed: %s", exc)
    ...
    if SESSION_SANDBOX_ENABLED and _resolved_mode:
        try:
            ...
            if workspace_record:
                ...
                transition(worktree_id=workspace_record["worktree_id"], owner_id=session.owner_id, status="entered")
    ...
    try:
        ...
        if repo_context:
            try:
                ...
                if workspace_record:
                    from coordination.workspace_lifecycle import transition
                    transition(worktree_id=workspace_record["worktree_id"], owner_id=session.owner_id,
                               status="finalized", diff_ref=f"swarm:{session.coordination_id}")
        ...
        if workspace_record:
            try:
                from coordination.workspace_lifecycle import transition
                transition(worktree_id=workspace_record["worktree_id"], owner_id=session.owner_id, status="exited")
