...
def build_android_project(uid: str, project_id: str, source_container_name: str = "dev_sandbox") -> dict:
    ...
    try:
        ...
        command = (
            ...
            "test -x ./gradlew && ./gradlew --no-daemon --stacktrace assembleRelease "
            "|| (test -f ./gradlew && chmod +x ./gradlew && ./gradlew --no-daemon --stacktrace assembleRelease)"
        )
        result = {}
        ...
        try:
            ...
            raise AndroidBuildError("Android build exceeded the 15-minute timeout") from exc
        else:
