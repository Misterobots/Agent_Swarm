...
def handle_pending_context(
    ...
    # -----------------------------------------------------------------------
    # 4a. design_confirmation — user asked to confirm before Design Studio runs
    # -----------------------------------------------------------------------
    if ctx_type == "design_confirmation":
        original = pending_ctx.get("prompt", "")
        from brooks import clear_context
        clear_context(session_id=session_id, owner_id=owner_id)
        if user_input.strip() == "DESIGN_YES":
            # User confirmed — prefix so church.py skips the confirmation gate
            result["user_input"] = f"[DESIGN_CONFIRMED] {original}"
            yield {"type": "log", "content": "[Context Manager] Design confirmed — launching Design Studio"}
        else:
            # User declined — prefix so church.py skips the keyword override
            result["user_input"] = f"[DESIGN_DECLINED] {original}"
            yield {"type": "log", "content": "[Context Manager] Design declined — routing normally"}
        return  # fall through to routing
