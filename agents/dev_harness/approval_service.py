"""Durable, owner-scoped approval decisions.

This module is deliberately independent of FastAPI and the DevHarness loop.
The loop may keep asyncio waiters in memory, but the request, decision and
audit trail live in Postgres so a process restart cannot turn an approval into
an implicit grant.
"""
...
def get_request_any_owner(call_id: str) -> dict[str, Any] | None:
    """Lookup only metadata needed to distinguish 404 from a cross-owner 403."""
    if not call_id:
        return None
    try:
        with _db() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                cur.execute("SELECT call_id, owner_id, decision, expires_at FROM dev_approval_requests WHERE call_id=%s", (call_id,))
                row = cur.fetchone()
    ...
    try:
        with _db() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                cur.execute("SELECT * FROM dev_approval_requests WHERE owner_id=%s AND call_id=%s", (owner_id, call_id))
                row = cur.fetchone()
                return dict(row) if row else None
    except Exception:
        ...
        with _db() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                ...
                if not row:
                    return None
                if row["decision"]:
                    return dict(row)  # idempotent duplicate; never changes owner/scope
                ...
                cur.execute("""
                    ...
                    WHERE call_id=%s AND owner_id=%s AND decision IS NULL
                    RETURNING *
...
def audit_outcome(*, owner_id: str, call_id: str, event: str, by: str = "system") -> bool:
    ...
    try:
        with _db() as conn:
            with conn.cursor() as cur:
                cur.execute("UPDATE dev_approval_requests SET audit = audit || %s::jsonb WHERE call_id=%s AND owner_id=%s",
                            (json.dumps([{"event": event, "at": int(time.time()), "by": by}]), call_id, owner_id))
        return True
    except Exception:
        logger.exception("approval audit persistence failed")
        return False
